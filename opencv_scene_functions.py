...
# 색감 변경 함수
def color_filter(img, color, scale): #  입력 변수는 영상, 변경할 색상, 변경할 비율
...
# 밝기 변경 함수
def set_brightness(img, scale):     # 입력변수는 여상, 밝기를 변경할 값
...
def set_size(img, scale):
    return cv2.resize(img, dsize=(int(img.shape[1]*scale), int(img.shape[0]*scale)),
